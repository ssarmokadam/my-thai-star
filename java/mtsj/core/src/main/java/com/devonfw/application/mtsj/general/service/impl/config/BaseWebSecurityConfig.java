...
import com.devonfw.module.security.jwt.config.JwtAuthenticationProvider;
 ...
 * configuration is by purpose designed most simple for authentication based on
 * Json Web Token.
 */
public abstract class BaseWebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	public void configure(HttpSecurity http) throws Exception {
		...
		http.csrf().disable().exceptionHandling().and().sessionManagement()
				...
				.antMatchers(unsecuredResources).permitAll().antMatchers(HttpMethod.POST, "/login").permitAll()
				.anyRequest().authenticated().and()
				// the api/login requests are filtered with the JWTLoginFilter
				.addFilterBefore(new JWTLoginFilter("/login", authenticationManager(),keyStoreAccess),
						UsernamePasswordAuthenticationFilter.class)
				// other requests are filtered to check the presence of JWT in header
				.addFilterBefore(new JwtAuthencticationFilter(keyStoreAccess, objectMapper),
						UsernamePasswordAuthenticationFilter.class);
	...
	public UserDetailsService userDetailsService() {
		...
		manager.createUser(User.builder().username("testuser").password("testuser").roles("Cook").build());
		return manager;
	}
